...
output "certificates_list" {
  value = var.cert_id == "all" && var.cert_domain == null ? [for cert in data.qwilt_cdn_certificates.detail.certificate : { domain = cert.domain, cert_id = cert.cert_id, csr_id = cert.csr_id }] : null
}
