...
output "publish_ops_list" {
  value = var.site_id != "all" && data.qwilt_cdn_sites.detail.publish_op != null && var.publish_id == "all" && var.revision_id == "all" ? [for publish_op in data.qwilt_cdn_sites.detail.publish_op : { publish_id = publish_op.publish_id, revision_id = publish_op.revision_id, operation_type = publish_op.operation_type, publish_status = publish_op.publish_status, publish_state = publish_op.publish_state, is_active = publish_op.is_active, target = publish_op.target }] : null
}
...
output "publish_ops_list_by_revision_id" {
  value = data.qwilt_cdn_sites.detail.publish_op != null && var.site_id != "all" && var.revision_id != "all" && var.publish_id == "all" ? [for publish_op in data.qwilt_cdn_sites.detail.publish_op : { publish_id = publish_op.publish_id, revision_id = publish_op.revision_id, operation_type = publish_op.operation_type, publish_status = publish_op.publish_status, publish_state = publish_op.publish_state, is_active = publish_op.is_active } if publish_op.revision_id == var.revision_id] : null
}
